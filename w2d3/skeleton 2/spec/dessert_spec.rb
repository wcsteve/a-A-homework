...
describe Dessert do
  ...
  let( :chef) { double("chef", :name => "blah") }
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ['vanilla', 'icecream', 'cherry on top']

      ingredients.each do |ingredient|
        dessert.add_ingredient(ingredient)
      end

      expect(dessert.ingredients).to eq(['vanilla', 'icecream', 'cherry on top'])
      dessert.mix!
      expect(dessert.ingredients).not_to eq(['vanilla', 'icecream', 'cherry on top'])
      dessert.ingredients.sort
      expect(dessert.ingredients.sort).to eq(['cherry on top', 'icecream', 'vanilla'])
    end
  end

  describe "#eat" do
    it "subtracts an amount from the quantity" do
      expect(dessert.eat(10)).to eq(5)
    end
